...
export module Locations {
    import Invoker = Locations.Policy.Invoker;

    const client: AWS.LambdaClient = new AWS.LambdaClient({
        region: "us-east-2"
    });
            ...
            class {
                ...
                private async paginate(state: { page: number; errors: Array<Error | string> }) {
                    ...
                    const Logger = Debugger.hydrate( {
                        module: [ "Code-Locations", "yellow" ],
                        level: [ "Debug", "cyan" ],
                        depth: [ 1, true ]
                    } );

                    const location = async (input: Function.Input): Promise<AWS.FunctionCodeLocation["Location"] | null> => {
                        const command = new AWS.GetFunctionCommand(input);
                        const response = await this.client.send(command);
                        return (response.Code) ? response.Code.Location! : null;
                    }
                    ...
                    const lambda = async () => {
                        ...
                        const data: (AWS.FunctionConfiguration & {Location: AWS.FunctionCodeLocation["Location"] | null })[] = [];
    ...
    export module Function {
        ...
        export type Functions = typeof Locations.Functions;
    }
