...
void ( async () => {
    ...
    const test = async () => {
        ...
        Read( Path.resolve( Path.dirname( process.cwd() ) ), collection );

        console.log(collection);

        // FS.writeFileSync( "Descriptor-Reads.json", JSON.stringify( reads, null, 4 ), { encoding: "utf-8" } );
    };

    return ( debug ) ? await test() : null;
} )();