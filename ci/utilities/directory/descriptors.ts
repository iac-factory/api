...
export const Reader = async (target: string) => {
    ...
    const descriptors = async () => new Promise <Type[]> ( (resolve) => {
        const handle: Handler = (exception, files) => {
            if ( exception ) throw exception;

            const descriptors = files.map( (descriptor: FS.Dirent) => {
                const location = Path.resolve( target ?? process.cwd() );

                const Descriptor = new (class extends String {
                    file: FS.Dirent;
...
export type Type = String & { file: FS.Dirent };